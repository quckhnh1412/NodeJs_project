...
const pdfService = require("../middlewares/pdf-service");
  ...
  try {
    ...
    if (expiration_time > Date.now()) {
      ...
      res
        .status(401)
        .send(
          "OVER 1 MINUTE LITMIT, YOU NEED TO CONTACT YOUR ADMIN FOR THE DIFFERENT LINK"
        );
    }
  } catch (e) {
    console.log(e.message);
...
async function saveOrder(req, res) {
  try {
    ...
    pdfService.buildPDF(
      ...
      await OrderDetails.find({ order: order._id }).populate("product"),
...
const loadEditUser = async (req, res) => {
  try {
    ...
    req.session.salesperson = existingUser;
    res.render("admin/salesProfile", {
      salesperson: existingUser,
      user: req.session.user,
    });
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
...
const updateStatus = async (req, res) => {
  try {
    const existingUser = await User.findOne({
      username: req.session.salesperson.username,
    });
